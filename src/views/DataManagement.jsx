...
export default function DataManagement() {
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="font-serif italic text-4xl text-walnut tracking-tight">Data</h1>
        <p className="text-toffee text-sm mt-1">Back up, restore, and configure.</p>
      </header>
        ...
        <div
          className={`rounded-2xl px-5 py-3 text-sm ${
            message.tone === 'error'
              ...
              : 'bg-sage-light/40 border border-sage/40 text-olive'
          }`}
        >
          {message.text}
        </div>
      ...
      <Section title="Backup">
        <div className="flex flex-wrap gap-3">
          ...
          <button
            ...
            className="px-5 py-2.5 bg-ebony text-cream-pale rounded-full font-medium hover:bg-charcoal"
          >
            Import JSON
          </button>
          <input ref={fileRef} type="file" accept="application/json" onChange={handleImportFile} className="hidden" />
        </div>
        <p className="text-xs text-camel mt-2">
      ...
      <Section title="Breville grinder range">
        <div className="grid sm:grid-cols-2 gap-3 max-w-md">
          <label className="block">
            <span className="block text-sm text-walnut mb-1">Min</span>
            <input
              type="number"
              min={1}
              value={settings.breville_grind_min}
              onChange={(e) => setSettings({ breville_grind_min: Number(e.target.value) || 1 })}
              className="w-full px-3 py-2 border border-walnut/20 rounded-lg bg-cream-pale text-walnut tabular-nums"
            />
          </label>
          <label className="block">
            <span className="block text-sm text-walnut mb-1">Max</span>
            <input
              type="number"
              min={1}
      ...
      <Section title="Danger zone">
        <button
          onClick={handleClear}
          className={`px-5 py-2.5 rounded-full font-medium text-white ${
            clearStage === 0
              ? 'bg-camel hover:bg-toffee'
              : clearStage === 1
...
function Section({ title, children }) {
  return (
    <section className="bg-cream-pale rounded-2xl border border-walnut/10 p-6 space-y-3">
      <h2 className="text-xs uppercase tracking-[0.2em] text-camel font-medium">{title}</h2>
      {children}
    </section>
  )
}
