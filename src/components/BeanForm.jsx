...
export default function BeanForm({ initial, onSave, onCancel }) {
  ...
  return (
    <form onSubmit={submit} className="bg-white border border-stone-200 rounded-lg p-5 space-y-4">
      <h3 className="text-lg font-medium">{initial?.id ? 'Edit bean' : 'Add bean'}</h3>
      ...
      <div className="flex gap-2 pt-2">
        ...
        <button
          type="button"
          onClick={onCancel}
          className="px-4 py-2 text-stone-600 rounded hover:bg-stone-100"
        >
          Cancel
        </button>
      </div>
